...
import static soongsil.kidbean.server.quiz.domain.type.QuizCategory.ANIMAL;
import static soongsil.kidbean.server.quiz.domain.type.QuizCategory.FOOD;
import static soongsil.kidbean.server.quiz.domain.type.QuizCategory.NONE;
import static soongsil.kidbean.server.quiz.domain.type.QuizCategory.OBJECT;
import static soongsil.kidbean.server.quiz.domain.type.QuizCategory.PLANT;
...
public class ImageQuizTuningTest {
    ...
    void setUp() {
        ...
        member = memberRepository.findByEmail("email1").get();
    }
    ...
    void selectRandomImageQuiz() {
        //given
        List<ImageQuizSolveListResponse> list = new ArrayList<>();
        int loopCnt = 1;
        ...
        for (int i = 0; i < loopCnt; i++) {
            list.add(imageQuizService.selectRandomImageQuizList(member.getMemberId(), 5));
        }

        stopWatch.stop();

        //then
        for (ImageQuizSolveListResponse response : list) {
...
//        //given
//        List<ImageQuizSolveResponse> resultList = new LinkedList<>();
//        int loopCnt = 1;
//        int threadCount = loopCnt;
//
...
//                try {
//                    resultList.addAll(imageQuizService.selectRandomImageQuizList(member.getMemberId(), 5)
//                            .imageQuizSolveResponseList());
//                } finally {
//                    latch.countDown();
//                }
//            });
...
//
//        //then
//        assertThat(resultList.size()).isEqualTo(loopCnt * 5);
//
//        log.info("반복 횟수: {} 회", loopCnt);
//        log.info("총 소요 시간: {} ms", stopWatch.getTotalTimeMillis());
//    }
