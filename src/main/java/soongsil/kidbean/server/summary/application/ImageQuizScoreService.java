...
public class ImageQuizScoreService {
    ...
    @Transactional
    public void addImageQuizScore(Member member, ImageQuiz imageQuiz, int addScore, boolean isExist) {
        ImageQuizScore imageQuizScore = imageQuizScoreRepository.findByMemberAndQuizCategory(member, imageQuiz.getQuizCategory())
                .orElseGet(() -> imageQuizScoreRepository.save(ImageQuizScore.makeInitImageQuizScore(member, imageQuiz.getQuizCategory())));
